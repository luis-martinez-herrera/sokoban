...
import sys
import pygame
import queue  # Changed Queue to queue
...
class game:
    ...
    def __init__(self, filename, level):
        self.queue = queue.LifoQueue()  # Changed Queue.LifoQueue to queue.LifoQueue
        self.matrix = []
        if level < 1:
            print("ERROR: Level " + str(level) + " is out of range")  # Added parentheses
            sys.exit(1)
        else:
            with open(filename, 'r') as file:  # Using 'with' statement
                level_found = False
                for line in file:
                    row = []
                    if not level_found:
                        ...
                        if line.strip() != "":
                            ...
                            for c in line:
                                if c != '\n' and self.is_valid_value(c):
                                    ...
                                    print("ERROR: Level " + str(level) + " has invalid value " + c)  # Added parentheses
                                    sys.exit(1)
    ...
    def set_content(self, x, y, content):
        if self.is_valid_value(content):
            ...
            print("ERROR: Value '" + content + "' to be added is not valid")  # Added parentheses
...
def display_box(screen, message):
    ...
    pygame.draw.rect(screen, (0, 0, 0),
                     ((screen.get_width() // 2) - 100,  # Integer division
                      (screen.get_height() // 2) - 10,  # Integer division
                      200, 20), 0)
    pygame.draw.rect(screen, (255, 255, 255),
                     ((screen.get_width() // 2) - 102,  # Integer division
                      (screen.get_height() // 2) - 12,  # Integer division
                      204, 24), 1)
    if len(message) != 0:
        screen.blit(fontobject.render(message, 1, (255, 255, 255)),
                    ((screen.get_width() // 2) - 100, (screen.get_height() // 2) - 10))  # Integer division
    pygame.display.flip()
...
def ask(screen, question):
    ...
    pygame.font.init()
    current_string = []
    display_box(screen, question + ": " + "".join(current_string))  # Changed string.join to "".join
    while 1:
        inkey = get_key()
        ...
        display_box(screen, question + ": " + "".join(current_string))  # Changed string.join to "".join
    return "".join(current_string)  # Changed string.join to "".join

def start_game():
    start = pygame.display.set_mode((320, 240))
    level = ask(start, "Select Level")
    try:  # Added try-except for converting string to int
        level = int(level)
        if level > 0:
            return level
        else:
            print("ERROR: Invalid Level: " + str(level))  # Added parentheses
            sys.exit(2)
    except ValueError:
        print("ERROR: Invalid Level: " + level)  # Added parentheses
        sys.exit(2)
